...
import numpy as np
import pandas as pd
from generator.workload_generator.distribution import (
...
from generator.workload_generator.sample_request import RequestFinder, load_requests
from generator.workload_generator.utils import (
    convert_to_stat_df,
    get_tokenizer,
    if_sessioned_dataset,
    load_config,
    make_serializable,
    plot_workload,
...
def generate_from_stat_csv(
    ...
    input_len_configs, output_len_configs, rps_configs = read_distribution_stats(
        merged_df
    )
    input_len_dist = []
    output_len_dist = []
    ...
    for rps_config in rps_configs:
        rps_segment = generate_poisson_dist(
            target=rps_config["mean_rps"],
            sample_size=rps_config["total_seconds"],
...
def generate_synthetic_from_dist(
    ...
    if input_token_len_dist is not None and output_token_len_dist is not None:
        if not (
            len(rps_dist) == len(input_token_len_dist) == len(output_token_len_dist)
        ):
            raise ValueError(
                f"All distributions must have the same length, len(rps_dist): {len(rps_dist)}, len(input_token_len_dist): {len(input_token_len_dist)}, len(output_token_len_dist): {len(output_token_len_dist)}"
            )
    workload = []
    current_time = 0
    ...
    while current_time < total_seconds * 1000:
        ...
        if input_token_len_dist is not None:
            current_input_len = (
                input_token_len_dist[time_idx] / input_scale
                if input_token_len_dist[time_idx]
                else None
            )
        if output_token_len_dist is not None:
            current_output_len = (
                output_token_len_dist[time_idx] / output_scale
                if output_token_len_dist[time_idx]
                else None
            )
        inter_arrival_time = (
            1000
            if current_rate == 0
            else np.random.exponential(scale=1000 / current_rate)
        )
...
def generate_synthetic(
    ...
    assert (
        duration_ms is not None and interval_ms is not None
    ...
    while interval < num_intervals:
        current_rate, previous_rate = sine_fluctuation(
            interval, qps_pattern_config, num_intervals, previous_rate
        )
        current_input_len = None
        current_output_len = None
        if input_pattern_config:
            current_input_len, previous_input_len = sine_fluctuation(
                interval, input_pattern_config, num_intervals, previous_input_len
            )
            current_input_len = current_input_len if current_input_len > 0 else 1
        if output_pattern_config:
            current_output_len, previous_output_len = sine_fluctuation(
                interval, output_pattern_config, num_intervals, previous_output_len
            )
...
def generate_from_azure_csv(
    ...
    if tracing_file_end_time < end_time:
        logging.warning(
            f"{tracing_file_end_time} can not cover duration {duration_ms}, cap to end time of tracing file"
        )
...
def main(args):
    ...
    tokenizer = get_tokenizer(
        pretrained_model_name_or_path=args.model, trust_remote_code=True
    )
    ...
    if args.trace_type == "synthetic":
        ...
        if args.traffic_pattern:
            qps_pattern_config = to_fluctuate_pattern_config(
                config_type=args.traffic_pattern, mean=6
            )
        elif args.traffic_pattern_config:
            qps_pattern_config = user_to_synthetic_config(
                user_config=load_config(args.traffic_pattern_config),
                duration_ms=args.duration_ms,
            )

        if args.prompt_len_pattern:
            input_pattern_config = to_fluctuate_pattern_config(
                config_type=args.prompt_len_pattern, mean=1024
            )
        elif args.prompt_len_pattern_config:
            input_pattern_config = user_to_synthetic_config(
                user_config=load_config(args.prompt_len_pattern_config),
                duration_ms=args.duration_ms,
            )

        if args.completion_len_pattern:
            output_pattern_config = to_fluctuate_pattern_config(
                config_type=args.completion_len_pattern, mean=1024
            )
        elif args.completion_len_pattern_config:
            output_pattern_config = user_to_synthetic_config(
                user_config=load_config(args.completion_len_pattern_config),
                duration_ms=args.duration_ms,
            )
        ...
        generated_workload = generate_synthetic(
        ...
        if args.trace_type == "constant":
            generated_workload = generate_constant(
                prompt_file_path=args.prompt_file,
                tokenizer=tokenizer,
                qps=args.target_qps,
            ...
            generated_workload = generate_from_stat_csv(
                prompt_file_path=args.prompt_file,
                duration_ms=args.duration_ms,
                tokenizer=tokenizer,
            ...
            generated_workload = generate_from_azure_csv(
    ...
    if workload_dict:
        ...
        for workload_name, workload in workload_dict.items():
            plot_workload(
                workload=workload, bin_size_sec=1, output_dir=f"{args.output_dir}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="File containing sampling prompts.",
    )
    parser.add_argument(
        "--trace-type",
        type=str,
        required=True,
        choices=["constant", "synthetic", "stat", "azure"],
    ...
    parser.add_argument(
        ...
        type=int,
        default=60000,
        help="Duration of the trace generated.",
    )
    parser.add_argument(
        "--group-interval-seconds",
        type=int,
        default=1,
        help="Grouping interval seconds.",
    )
    parser.add_argument(
        "--stat-trace-type",
        type=str,
        choices=["maas", "cloudide"],
        default="maas",
        help="Type of stat traces.",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        required=False,
        default="output",
        help="Output directory to save." "the workload.",
    )
    parser.add_argument(
        "--output-format",
        type=str,
    ...
    parser.add_argument(
        ...
        required=False,
        default=1,
        help="Target QPS for the workload.",
    )
    parser.add_argument(
        "--target-prompt-len",
        type=int,
        required=False,
        default=None,
        help="Target prompt length for the workload.",
    )
    parser.add_argument(
        "--target-completion-len",
        type=int,
        required=False,
        default=None,
        help="Target completion length for the workload.",
    )
    parser.add_argument(
        "--traffic-pattern",
        type=str,
    ...
    parser.add_argument(
        ...
        required=False,
        default=1.0,
        help="Input length scaling factor.",
    )
    parser.add_argument(
        "--output-scale",
        type=float,
        required=False,
        default=1.0,
        help="Output length scaling factor.",
    )
    parser.add_argument(
        ...
        required=False,
        default=1,
        help="Maximum number of overlapping sessions.",
    )
