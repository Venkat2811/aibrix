#!/usr/bin/env python3
"""VTC Benchmark Analysis Script - Comprehensive analysis of VTC routing performance."""
...
# Import query_prometheus from bench_and_collect_stats to avoid duplication
from bench_and_collect_stats import query_prometheus
...
class VTCBenchmarkAnalyzer:
    ...
    def _extract_timeframe(self) -> Tuple[datetime, datetime]:
        """Extract benchmark timeframe from request files or comprehensive stats."""
        request_files = sorted(self.results_dir.glob("request_*.json"))
        all_timestamps = []

        # Try to get timestamps from individual request files first
        if request_files:
            ...
            # Fallback to comprehensive stats if no individual files exist
            if "request_data" in self.comprehensive_stats:
                for req in self.comprehensive_stats["request_data"]:
                    # Extract timestamp from send_time if available
                    if "send_time" in req:
                        timestamp = datetime.fromtimestamp(req["send_time"])
                        all_timestamps.append(timestamp)

            # If still no timestamps, use current time as fallback
            if not all_timestamps:
                now = datetime.now()
                all_timestamps = [now, now]
    ...
    def query_prometheus_range(
        ...
        """Query Prometheus for time range metrics data using the imported function."""
        ...
        try:
            ...
            if "data" not in result:
                print(f"Warning: Prometheus query returned unexpected format")
                return {"data": {"result": []}}
            return result
        except requests.exceptions.RequestException as e:
            print(f"Warning: Prometheus connection failed: {e}")
            return {"data": {"result": []}}
    ...
    def analyze_vtc_bucket_evolution(self) -> Dict[str, Any]:
        ...
        result = self.query_prometheus_range("vtc_bucket_size_active", step="30s")
        bucket_data = {}
    ...
    def _analyze_bucket_adaptation(self, bucket_data: Dict) -> Dict[str, Any]:
        ...
        if total_range < 50:
            ...
            status = "stable_good"
            recommendations = ["VTC bucket sizes adapted well and stabilized"]
        elif avg_std > 100:
            ...
            recommendations = ["VTC showed moderate adaptation behavior"]
    ...
    def analyze_ttft_performance(self) -> Dict[str, Any]:
        ...
        for percentile in [50, 90, 99]:
            ...
            result = self.query_prometheus_range(query, step="60s")
    ...
    def analyze_pod_utilization(self) -> Dict[str, Any]:
        ...
        print("\nAnalyzing pod utilization...")

        pod_distribution = {"vtc-basic": {}, "random": {}}

        # Try individual request files first
        request_files = list(self.results_dir.glob("request_*.json"))
        if request_files:
            for request_file in request_files:
                with open(request_file) as f:
                    data = json.load(f)
            ...
            if "request_data" in self.comprehensive_stats:
                for req in self.comprehensive_stats["request_data"]:
                    ...
                    if algorithm in pod_distribution:
                        if pod not in pod_distribution[algorithm]:
                            pod_distribution[algorithm][pod] = 0
                        pod_distribution[algorithm][pod] += 1
    ...
    def generate_fairness_analysis(self) -> Dict[str, Any]:
        ...
        improvements = [
            metrics.get("latency_improvement_pct", 0)
            for metrics in fairness_comparison.values()
        ]
        ...
        if overall_improvement < -5:
            recommendations.extend(
                [
                    ...
                    "Consider adjusting bucket size thresholds",
                ]
            )
    ...
    def generate_tuning_recommendations(
        ...
        if fairness_analysis.get("overall_improvement", 0) < -10:
            recommendations.extend(
                [
                    "CRITICAL: VTC fairness is degraded",
